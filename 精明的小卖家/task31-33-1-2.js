...
var tableView = {
  ...
  renderCase1: function render(arr, winSelect, loserSelect) {
    ...
    template = template.replace(
      /{{thead-firstTd-content}}/g,
      winSelect.typeText
    );
    template = template.replace(
      /{{thead-secondTd-content}}/g,
      loserSelect.typeText
    );
    arr.forEach(function(element) {
      ...
      if (flag == 0) {
        trContent +=
          ...
          arr.length +
          "' >" +
          element[loserSelect.type] +
          "</td>";
        flag = 1;
      }
      trContent += "<td>" + element[winSelect.type] + "</td>";
